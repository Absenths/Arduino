...
void makeCircle(int xin, int yin){
    ...
    //Write to immediate neighbors
    for( int x=-1; x<2; x++){
      for(int y=-1; y<2; y++){
        int xcord = xin+x;
        int ycord = yin+y;
        if((xcord>=0)&&(ycord>=0)&&(xcord<xySize)&&(ycord<xySize)){
          globalXY[xcord][ycord] = constrain(globalXY[xcord][ycord] + globalXY[xin][yin]/10,0,lvl1);
        }
      }
    }
...
//    int yin = (xySize/2*distance/MAX_DIST)*sin((angle+270)*3.14/180)+xySize/2; // old form
    ...
    if((xin > 60 || xin < 0 || yin > 60 || yin < 0)){
        ...
        globalXY[xin][yin] = constrain(globalXY[xin][yin]+lvl2,0,lvl1); //increase the value in detected pixels
        makeCircle(xin,yin);
    }
...
void displayColor(float distance, float angle, bool startBit, byte quality)
{
  ...
  if(quality > 10 && distance < MAX_DIST && distance > MIN_DIST){
    goodReadCount++;
  ...
  if(writeReady && angle>writeAngle && goodReadCount > 100){
